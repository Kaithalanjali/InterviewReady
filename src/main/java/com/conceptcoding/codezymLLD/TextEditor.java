...
public class TextEditor {
    ...
    class AddCommand implements Command{
        ...
        AddCommand(int r, int c, String t){
            row=r;
            col=c;
            text=t;
        }
    ...
    class DeleteCommand implements Command{
        ...
        DeleteCommand(int r, int c,int l){
            row=r;
            col=c;
            length=l;
        }
        ...
        public void undo(){
            rows.get(row).insert(col,deleted);
        }
    }

    /**
     * Add text at specified row and column
     * Time Complexity: O(T + R) where T = length of text, R = rows to create if needed
     * Space Complexity: O(T) for storing the text
     */
    public void addText(int row, int column, String text){
        ...
        Command cmd = new AddCommand(row,column,text);
        cmd.execute();
        undoStack.push(cmd);
        redoStack.clear();
    }
    ...
    /**
     * Delete text from specified row starting at column for given length
     * Time Complexity: O(L) where L = length of text to delete
     *   - StringBuilder delete operation: O(L)
     *   - Stack operations: O(1)
     * Space Complexity: O(L) for storing deleted text in command
     */
    public void deleteText(int row, int startColumn, int length){
        if(row >= rows.size()){
            return; // Row doesn't exist, nothing to delete
        }
        Command cmd = new DeleteCommand(row,startColumn,length);
        cmd.execute();
        undoStack.push(cmd);
        redoStack.clear();
    }
     ...
     * Time Complexity: O(T) where T = length of text in the operation being undone
     *   - Stack pop/push: O(1)
     *   - Command undo (insert/delete on StringBuilder): O(T)
     * Space Complexity: O(1) - moves command from one stack to another
     */
    public void undo(){
     ...
     * Time Complexity: O(T) where T = length of text in the operation being redone
     *   - Stack pop/push: O(1)
     *   - Command execute (insert/delete on StringBuilder): O(T)
     * Space Complexity: O(1) - moves command from one stack to another
     */
    public void redo(){
    ...
    /**
     * Read the content of a specific row
     * Time Complexity: O(L) where L = length of the row's text
     *   - List access: O(1)
     *   - StringBuilder toString: O(L)
     * Space Complexity: O(L) - creates new String from StringBuilder
     */
